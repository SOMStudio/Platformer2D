...
public class GameController_Plt2D : BaseGameController
{
	...
	[Header("Managers")]
	[SerializeField] private SpawnPlatforms_Plt2D spawnManager;
		...
		if (startGame)
		{
			if (playerGO1)
			{
				...
				if (
					...
					&& ((playerPos - firstPlatformPos).magnitude > spawnManager.HorizontalMax * 2) // check distance
				)
				{
					// player not control
	...
	private void Init()
	{
		...
		// grab a ref to the player's game-object for later
